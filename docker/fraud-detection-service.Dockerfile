...
RUN apt-get update && apt-get install -y --no-install-recommends \
        build-essential gcc libffi-dev gfortran libopenblas-dev && rm -rf /var/lib/apt/lists/*
WORKDIR /build
...
RUN pip wheel --wheel-dir /wheels /build/shared && \
    ...
    pip install --prefix=/install --no-cache-dir \
        ...
        structlog bleach tenacity

FROM gcr.io/distroless/python3-debian12:nonroot
ENV PYTHONPATH=/install/lib/python3.11/site-packages:/app \
    SB_SERVICE_NAME=fraud-detection-service SB_BIND_PORT=8004
COPY --from=builder /install /install
COPY src/fraud-detection-service /app
